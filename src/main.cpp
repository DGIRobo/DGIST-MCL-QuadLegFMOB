...
int flag_GRF = 0;           // flag for switching GRF Observer 0: FOB, 1: MOB

double sensor_cutoff = 100; // 10
double PID_pos_cutoff = 150;
double RWDOB_cutoff = 150;
double FOB_cutoff = 100;
double MOB_gain = 2*pi*100;
...
void mycontroller(const mjModel* m, mjData* d)
{
    ...
    // Force Observer
    //printf("----------Comparing Ground Reaction Force---------- \n");
    double grf_x = d->sensordata[9];
    double grf_y = d->sensordata[10];
    double grf_z = d->sensordata[11];
    double cartesian_grf_x = grf_x * cos(pi - state_Model_FL.q_bi[1]) - grf_y * sin(pi - state_Model_FL.q_bi[1]);
    double cartesian_grf_y = grf_x * sin(pi - state_Model_FL.q_bi[1]) + grf_y * cos(pi - state_Model_FL.q_bi[1]);
    double grf_r = cartesian_grf_y * cos(state_Model_FL.posRW[1] - pi / 2) + cartesian_grf_x * sin(state_Model_FL.posRW[1] - pi / 2);

    ctrl_FL.FOBRW(&state_Model_FL, FOB_cutoff); // Rotating Workspace Force Observer (RWFOB)
    ctrl_FL.MOBRW(&state_Model_FL, MOB_gain); // Rotating Workspace Momentum Observer (RWMOB)

    //Admittance Control
    ctrl_FL.admittanceCtrl(&state_Model_FL, omega_n, zeta, k, flag_admitt, flag_GRF); //parameter(omega_n,zeta,k)

    // PID Control
    ctrl_FL.pid_gain_pos(p_gain, d_gain, PID_pos_cutoff); //(kpr, kdr, kpt, kdt,freq)
   ...
   // Torque input Biarticular
    ...
    if (loop_index % data_frequency == 0) {
        save_data(m, d, &state_Model_FL, &ctrl_FL);
    }
    loop_index += 1;
}
